...
bbox_data = pickle.load(open('C:/Users/spate/Downloads/out.pkl',"rb"))
...
pcloud = np.fromfile(images[1].replace('_image.jpg', '_cloud.bin'), dtype=np.float32)
pcloud = pcloud.reshape([3, -1])

proj = np.fromfile(images[1].replace('_image.jpg', '_proj.bin'), dtype=np.float32)
proj.resize([3, 4])
...
y1 = detection_boxes[1][0]*w
x1 = detection_boxes[1][1]*h
y2 = detection_boxes[1][2]*w
x2 = detection_boxes[1][3]*h
